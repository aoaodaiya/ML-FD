...
def cn_op_2ins_space_chan(
    x, crop="neither", beta=1, bbx_thres=0.1, lam=None, chan=False
):
...
class SelfNorm(nn.Module):
    ...
    def __init__(self, chan_num, is_two=False):
        ...
        self.g_fc = nn.Conv1d(
            chan_num, chan_num, kernel_size=2, bias=False, groups=chan_num
        )
    ...
    def forward(self, x):
        ...
        if self.f_fc is not None:
            ...
            return x * g_y.expand_as(x) + mean.expand_as(x) * (
                f_y.expand_as(x) - g_y.expand_as(x)
            )
        else:
